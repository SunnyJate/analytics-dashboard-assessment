...
const EVDashboard = () => {
  ...
  const pieChartData2 = Object.keys(evTypeCounts).map((evType) => ({
    ...
    percentage: ((evTypeCounts[evType] / displayedData.length) * 100).toFixed(
      2
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <section className="mb-4 grid md:grid-cols-4 sm:gr id-cols-2 gap-4">
        ...
        {/* <div className="displayFlex-justify center"> */}
          <div className="metric-box bg-yellow-100 hover:bg-yellow-300 transition-colors duration-300 p-6 rounded-lg shadow-lg flex flex-col items-center text-center transform hover:scale-[1.02]">
            <FaCar className="icon text-blue-400 text-2xl mb-2" />
          ...
          <div className="metric-box bg-red-200 hover:bg-red-300 transition-colors duration-300 p-6 rounded-lg shadow-lg flex flex-col items-center text-center transform hover:scale-[1.02]">
            <FaDollarSign className="icon text-purple-500 text-xl mb-2" />
            <h2 className="font-bold text-base">Price Range</h2>
            <p className="text-xl font-semibold">
              {Math.round(
                Math.min(...filteredData.map((item) => item.baseMsrp))
              )}{" "}
              -
              {Math.round(
                Math.max(...filteredData.map((item) => item.baseMsrp))
              )}{" "}
              USD
            </p>
          </div>
      ...
      {/* Charts Section */}
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-gray-500">
        <PieChart pieChartData2={pieChartData2} COLORS={COLORS} />

        <Chart displayedData={chartData} />
      </section>
